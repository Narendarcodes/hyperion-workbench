...
/**
 * Browser language a page advertises to boot into the product's English-only
 * surface: with no stored preference the client derives its initial locale
 * from the browser, and Playwright's default browser already asks for
 * English, so this names that default explicitly.
 */
 ...
 * The context uses Asia/Shanghai to preserve
...
export async function newEnglishPage(browser: Browser, height = 1000): Promise<Page> {
  return await browser.newPage({ viewport: { width: 1680, height }, locale: EN_BROWSER_LOCALE, timezoneId: 'Asia/Shanghai' })
}
