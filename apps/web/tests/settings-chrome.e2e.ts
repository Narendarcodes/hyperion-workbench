...
// The English surface: the product ships English only, so every browser resolves here.
...
describe('web e2e: settings modal and General preferences', () => {
  ...
  beforeAll(async () => {
    ...
    // English browser: the shared page asserts the English settings surface
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    // The shipped default's English display name comes from the en dictionaries.
  ...
  it('renders English on every browser and persists the language preference', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-settings-language'))
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: 'Settings' })
    await dialog.waitFor({ timeout: 10_000 })
    // The document language follows the active locale in the assembled app.
    // The product ships English only, so every browser resolves to en —
    // asserting it here is what makes the check discriminating.
    expect(await page.evaluate(() => document.documentElement.lang)).toBe('en')
    // The Language selector pill shows the active locale's own name, and its
    // menu offers exactly the one shipped language.
    const selector = dialog.getByRole('button', { name: 'English' })
    expect(await selector.getAttribute('aria-haspopup')).toBe('menu')
    await selector.click()
    await page.getByRole('menuitem', { name: 'English' }).waitFor({ timeout: 5_000 })
    expect(await page.getByRole('menuitem').count()).toBe(1)
    // The settings-owned copy stays English throughout: dialog title, nav,
    // Appearance labels.
    await expect.poll(() => dialog.getByText('Appearance', { exact: true }).count(), { timeout: 5_000 }).toBe(1)
    expect(await page.evaluate(() => localStorage.getItem('dsh.locale'))).toBeNull()
    // Selecting the active language still persists the explicit preference:
    // the active value may be provisional (browser-derived), so the write is
    // unconditional.
    ...
    // Another port shares the same Host settings home, so it receives the
    // explicit English preference too.
    const second = await launchWebScaffold({ harnessHome: scaffold.harnessHome })
    const secondPage = await browser.newPage({ viewport: { width: 1680, height: 1000 }, locale: EN_BROWSER_LOCALE })
    const secondTripwire = watchConsole(secondPage)
    try {
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    // distinguish detection from the default — the document-lang assertion
    // below pins that the locale sync ran instead of echoing the markup.
  ...
  it('opens a browser asking for no shipped language in English', async () => {
    // The product default for "no usable signal": a French browser asks
    // for no shipped language, so resolution falls to FALLBACK_LOCALE (en).
    const fresh = await launchWebScaffold({})
    ...
    try {
      ...
      // Golden of the English dialog — the visible output this surface produces.
  ...
  it.skipIf(MODE === 'record')('keeps the fixture inventory closed', async () => {
    ...
    await assertFixtureInventory(SNAPSHOT_DIR, ['dialog-en.expected.md', 'plugins.expected.md'])
  })
})
