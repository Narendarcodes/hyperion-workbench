...
- button "Process details I will inspect the current Session's dynamic Cordis Plugins before defining the snapshot Package.":
  - img
  - img
  - text: Process details I will inspect the current Session's dynamic Cordis Plugins before defining the snapshot Package.
- 'button "Tool call cordis_inspect_self · {}"':
  - img
  - img
  - text: "Tool call cordis_inspect_self · {}"
- button "Process details No dynamic Plugins are present, so I will define the requested Host and Client Package.":
  - img
  - img
  - text: Process details No dynamic Plugins are present, so I will define the requested Host and Client Package.
...
- button "Inspect"
- button "Process details The Host returned snap-1/pkg-1, so I will request its first activation.":
  - img
  - img
  - text: Process details The Host returned snap-1/pkg-1, so I will request its first activation.
- img
- text: Run Cordis Plugin snap-1 · pkg-1 Ready
- button "Inspect"
- text: snap-1/pkg-1 is awaiting user approval (run-1).
- button "Process details The activation request has been submitted, so I will return the requested readiness marker.":
  - img
  - img
  - text: Process details The activation request has been submitted, so I will return the requested readiness marker.
