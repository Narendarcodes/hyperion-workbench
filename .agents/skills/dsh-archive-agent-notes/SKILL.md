...
- **Rejected — delete:** delete the whole pair when the rejected idea is obsolete, superseded, no longer plausible, or unlikely to prevent re-litigation. Repair or delete inbound links.
...
1. Move the complete `foo.md` and `foo.i18n.yaml` pair from `implemented/<kind>/` to `archived/<kind>/`; `implemented` is deliberately absent from the archive path.
2. Make no body edits. Insert only `Archived: YYYY-MM-DD` immediately below `Status: implemented`, using the archival date.
3. Re-record the sidecar hash mechanically for the metadata-only edit. Do not reformat, update facts, or repair links inside the note.
...
5. Run `pnpm run verify-archived-agent-notes --write`. Its append-only mode first proves every existing seal still matches, then adds only the new pair hashes. Run the normal verifier afterward.

After the pair is sealed, never edit, move, reformat, or delete it. Archived notes remain valid inbound-link targets but are historical snapshots, not authority for current behavior.
