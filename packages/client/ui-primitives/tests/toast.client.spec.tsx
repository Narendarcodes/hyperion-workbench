...
describe('Toast', () => {
  it('announces its text and reports done after the hold-and-fade lifetime', () => {
    ...
    try {
      const onDone = vi.fn()
      const view = render(<Toast text="最多添加 50 张Image" icon={<svg data-testid="icon" />} onDone={onDone} />)
      const banner = view.getByRole('alert')
      expect(banner.textContent).toContain('最多添加 50 张Image')
  ...
  it('holds for the owner\'s window and hands the stylesheet the same value', () => {
    ...
    try {
      const onDone = vi.fn()
      const view = render(<Toast text="切换Failed" holdMs={6000} onDone={onDone} />)
