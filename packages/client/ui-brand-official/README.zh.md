---
description: "面向侧栏的 Hyperion 产品品牌填充，仅在官方构建中生效；供选择或替换品牌呈现的用户与维护者阅读。"
...
本包向侧栏品牌槽位——`sidebar.brand.mark` 与 `sidebar.brand.name`——填充 Hyperion 标志与名称。它只在客户端以 `official` profile 构建时注册这些填充；其余构建同样加载插件但不注册任何内容，因此外壳回退保持可见。会话首屏槽位（`conversation.hero.brand.mark`）在所有构建中都保持无填充：其声明包以 Hyperion 标志作为回退渲染。当部署身份就是 Hyperion 自身时选择本包；自有品牌的部署改为在相同槽位中组合另一个包。它不保留任何运行时状态，也不向模型请求贡献任何内容。
...
在身份为 Hyperion 自身的部署的浏览器名单中挂载本插件，然后以 `official` profile 构建客户端，让填充得以注册。

### 选择 profile

`DSH_CLIENT_BUILD_PROFILE` 决定渲染哪个品牌。`official` 构建在侧栏显示 Hyperion 标志与名称；任何其他取值都让外壳回退——Hyperion 标志与产品标签——保持原样。会话首屏无论 profile 如何都显示来自 `dsh-client-ui-conversation` 的 Hyperion 标志，因为这个回退本身就是产品标志。两种情况下插件都会照常加载并通过校验；只有注册受 profile 门控。
