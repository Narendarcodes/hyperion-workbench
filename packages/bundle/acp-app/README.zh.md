...
随附配置项使用自主 `local` 路由与 `hyperion-default` 模型创建 session；后续 patch 可以替换该配置项的完整 config。base profile 负责适配器、工具、持久化、策略、settings 与 credentials；ACP client 为每个 session 提供工作区。
