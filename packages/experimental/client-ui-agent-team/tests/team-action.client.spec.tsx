...
describe('TeamAction', () => {
  ...
  it('keeps a created task newer than an in-flight refresh', async () => {
    const stale = Promise.withResolvers<TeamActionResult<TeamView>>()
    const createdTask = { ...task, id: TASK_2, subject: 'Created task' }
    const load = vi.fn()
      .mockResolvedValueOnce({ ok: true, value: view })
      .mockImplementationOnce(() => stale.promise)
    ...
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Created task' } })
  ...
  it('tracks simultaneous create and task mutations independently', async () => {
    ...
    const complete = screen.getByRole<HTMLButtonElement>('button', { name: /Complete/ })
  ...
  it('creates a task from normalized blocker and write-scope lists', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: ' Details ' } })
    fireEvent.change(screen.getByPlaceholderText(/Blocking task ids/), { target: { value: 'task-1, task-1' } })
    fireEvent.change(screen.getByPlaceholderText(/Write scopes/), { target: { value: 'src/a, src/b' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
    await waitFor(() => {
  ...
  it('assigns, edits, completes, reopens, and deletes with contiguous CAS revisions', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Blocking task ids/), { target: { value: 'task-0' } })
    fireEvent.change(screen.getByPlaceholderText(/Write scopes/), { target: { value: 'src/runtime' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
