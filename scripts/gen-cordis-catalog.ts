...
/**
 * The curated partition maps {@link walkPartitionProblems} enforces, fail-closed
 * in both directions for services AND events: a rendered key/scope must be mapped
 * to a page, a mapped key/scope must still render, and — the backstop — a DECLARED
 * key/event the projection cannot see must carry a named walk exemption (a rendered
 * one must not). A third direction guards the scan itself: everything rendered must
 * also be declared to the scan, so a scan blind spot cannot decay silently. Pure so the
